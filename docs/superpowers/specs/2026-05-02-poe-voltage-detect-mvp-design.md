...
R_BOTTOM = 51 kOhm
GPIO 串联保护电阻 = 47 kOhm
分压系数 K = (R_TOP + R_BOTTOM) / R_BOTTOM = 20.6078
ADC 输入估算:
  48 V -> 2.33 V
  57 V -> 2.77 V
```

后续代码应按当前实物值优先设置 `POE_DIVIDER_RATIO = 20.6078`。如果改回 510 kOhm / 56 kOhm 方案，再同步修改代码常量。

推荐保护和滤波：

```text
ADC midpoint -> 47 kOhm -> ESP32 ADC pin
...
当前已经停止 AD8310/AD9959 方案，PoE MVP 可以复用旧实验引脚。复用前需要确认旧 AD8310 TPVout 分压线、旧 AD9959 控制排线已经从 ESP32 侧断开，避免同一 GPIO 同时接两套外部电路。
...
```text
GPIO2  = 旧 AD8310 TPVout ADC，改作 PoE Mode A ADC
GPIO13 = 旧 AD9959 SCK，改作 PoE Mode B ADC
GPIO14 = 旧 AD9959 SD0，空闲备用
GPIO15 = 旧 AD9959 CS，空闲备用
GPIO16 = 旧 AD9959 UP / IO_UPDATE，空闲备用
GPIO21 = 旧 AD9959 RST，空闲备用
```

如果现场发现 GPIO2 或 GPIO13 焊接不方便，备选方案是 `GPIO3` 先做单路 Mode A 验证，或者后续加 ADS1115 外置 ADC 做 A/B 双路检测。
...
```text
POE_DIVIDER_RATIO = 20.6078
POE_BRIDGE_DROP_COMP = 1.2 V
POE_ADC_SAMPLES = 120
```

桥压降补偿只用于显示。模式判定应以换算后的 `poe_v` 为准，但阈值留足余量，避免二极管压降导致边界误判。USB 5 V 低压验证时，二极管压降占比很高，换算结果只用于调试，不作为 PoE 电压精度结论。
...
5. A/B 同时输入 48 V：mode=A+B，status=OK
6. 限流电源 57 V 接 Mode A：GPIO2 前端 ADC 点不超过 3.3 V，mode=A，status=OK
7. 限流电源 57 V 接 Mode B：GPIO13 前端 ADC 点不超过 3.3 V，mode=B，status=OK
8. 模拟超过 60 V 前先断开 ESP32，只用万用表确认 ADC 点不会超过 3.3 V
```
